...
public class StribogHash {
    ...
    // --- TAU (перестановка байтов) ---
    private static final int[] TAU = new int[] {
    ...
    // --- Таблица A (64 64-битных слова) ---
    // Взята из RFC 6986 (значения в hex). Эти значения используются для L-преобразования.
    ...
    // --- Константы C[1..12] --- (каждая — 64 байта)
    // Значения копированы из RFC 6986 (представлены здесь в виде байтов)
    private static final byte[][] C = new byte[][] {
            ...
            new byte[] {
                    ...
                    (byte)0xd8,(byte)0xb7,(byte)0x13,(byte)0x33,(byte)0x39,(byte)0x35,(byte)0x20,(byte)0x3b,
                    (byte)0xe3,(byte)0x45,(byte)0x3e,(byte)0xaa,(byte)0x19,(byte)0x3e,(byte)0x83,(byte)0x7f,
                    (byte)0x12,(byte)0x20,(byte)0xcb,(byte)0xeb,(byte)0xc8,(byte)0x4e,(byte)0x3d,(byte)0x12
            },
            new byte[] {
...
// ------------------ Базовые операции ------------------
    ...
    // S: нелинейная замена каждого байта через PI
    private static void subBytes(byte[] state) {
        for (int i = 0; i < state.length; i++) state[i] = PI[state[i] & 0xFF];
    }

    // P: перестановка байтов по TAU
    private static byte[] permute(byte[] state) {
        byte[] out = new byte[state.length];
        for (int i = 0; i < 64; i++) out[i] = state[TAU[i]];
        return out;
    }

    // L: умножение 64-битных слов на матрицу A (фактически — побитовое суммирование строк A)
// Вход: state[0..63] — 64 байта. Разбиваем на 8 слов (по 8 байт), для каждого слова вычисляем
// l(word) = XOR_{t=0..63} (if bit t of word == 1 then A[63 - t] else 0)
    private static byte[] linearTransform(byte[] state) {
        byte[] out = new byte[64];
        for (int i = 0; i < 8; i++) {
            long acc = 0L;
            // каждый блок — 8 байт, индекс базового байта = i*8
            for (int j = 0; j < 8; j++) {
                int b = state[i * 8 + j] & 0xFF;
                for (int bit = 0; bit < 8; bit++) {
                    if (((b >>> bit) & 1) != 0) {
                        int idx = j * 8 + bit; // позиция бита в 0..63
                        acc ^= A[63 - idx];
                    }
                }
            }
            // Записываем acc в out в порядке little-endian (младший байт в начале)
            for (int k = 0; k < 8; k++) {
                out[i * 8 + k] = (byte) ((acc >>> (8 * k)) & 0xFF);
            }
        }
        return out;
    }

    // Полная композиция LPS: сначала S, затем P, затем L
    private static byte[] LPS(byte[] state) {
        byte[] s = Arrays.copyOf(state, state.length);
        subBytes(s);
        s = permute(s);
        s = linearTransform(s);
        return s;
    ...
    // E(K,m) — AES-подобная блочная функция
    private static byte[] E(byte[] K, byte[] m) {
        byte[][] roundKeys = new byte[13][];
        roundKeys[0] = Arrays.copyOf(K, 64);
        for (int i = 1; i < 13; i++) {
            byte[] tmp = xorBytes(roundKeys[i - 1], C[i - 1]);
            roundKeys[i] = LPS(tmp);
        }
        ...
        state = xorBytes(state, roundKeys[12]);
        return state;
    }

    // g_N — компрессорная функция
    private static byte[] g_N(byte[] h, byte[] N, byte[] m) {
        byte[] K = LPS(xorBytes(h, N));
        byte[] e = E(K, m);
        return xorBytes(xorBytes(e, h), m);
    }

    // Паддинг: формируем 64-байтный блок из остатка сообщения
    private static byte[] padBlock(byte[] tail) {
        byte[] m = new byte[64];
        System.arraycopy(tail, 0, m, 0, tail.length);
        if (tail.length < 64) m[tail.length] = (byte) 0x01;
        return m;
    }

    // Вспомогательные операции сложения по модулю 2^512 (little-endian)
    ...
    private static byte[] addBitsToN(byte[] N, int bits) {
        byte[] out = Arrays.copyOf(N, N.length);
        int carry = bits;
        int i = 0;
    ...
    private static byte[] toLittleEndianBits(int bits) {
        byte[] out = new byte[64];
        out[0] = (byte) (bits & 0xFF);
        out[1] = (byte) ((bits >>> 8) & 0xFF);
        out[2] = (byte) ((bits >>> 16) & 0xFF);
        out[3] = (byte) ((bits >>> 24) & 0xFF);
        return out;
    }
...
// ------------------ Публичные методы ------------------
    ...
    public static byte[] digest512(byte[] message) {
        byte[] h = new byte[64]; // IV = 0
        byte[] N = new byte[64];
        byte[] Sigma = new byte[64];

        int fullBlocks = message.length / 64;
        for (int i = 0; i < fullBlocks; i++) {
            byte[] m = Arrays.copyOfRange(message, i * 64, i * 64 + 64);
            h = g_N(h, toLittleEndianBits(512), m);
            N = addBitsToN(N, 512);
            Sigma = add512(Sigma, m);
        }
        int rem = message.length - fullBlocks * 64;
        if (rem > 0) {
            byte[] tail = Arrays.copyOfRange(message, fullBlocks * 64, message.length);
            byte[] m = padBlock(tail);
            h = g_N(h, toLittleEndianBits(rem * 8), m);
            N = addBitsToN(N, rem * 8);
            Sigma = add512(Sigma, m);
        }
        h = g_N(h, new byte[64], N);
        h = g_N(h, new byte[64], Sigma);
        return h;
    }
    ...
    public static byte[] digest256(byte[] message) {
        ...
        h = g_N(h, new byte[64], N);
        h = g_N(h, new byte[64], Sigma);
        return Arrays.copyOfRange(h, 0, 32);
    }
    ...
    // ------------------ Демонстрация ------------------
    public static String encryption512(String message) {
        byte[] m = message.getBytes(StandardCharsets.US_ASCII);
        byte[] h512 = digest512(m);
        return toHex(h512);
    }

    public static String encryption256(String message) {
        byte[] m = message.getBytes(StandardCharsets.US_ASCII);
        byte[] h256 = digest256(m);
        return toHex(h256);
    }
}
