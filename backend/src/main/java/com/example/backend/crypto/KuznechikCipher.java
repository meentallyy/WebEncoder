...
public class KuznechikCipher {
    ...
    // массив для хранения ключей (каждый ключ — 16 байт; iter_key[0..9])
    static byte[][] iter_key = new byte[10][16];
    ...
    static public void GOST_Kuz_Expand_Key(byte[] key_1, byte[] key_2)
    {
        int i;
    ...
    static public byte[] GOST_Kuz_Encrypt(byte[] blk)
    {
        int i;
    ...
    static public byte[] GOST_Kuz_Decrypt(byte[] blk)
    {
        int i;
        byte[] out_blk = new byte[BLOCK_SIZE];
        out_blk = blk;
    ...
    static public String printHexBinary(byte[] bytes) {
        return HexFormat.of().withUpperCase().formatHex(bytes);
    }


    static private void incCounter(byte[] counter) {
    ...
    static public byte[] GOST_Kuz_CTR_Process(byte[] ivCounter, byte[] input) {
        if (ivCounter.length != BLOCK_SIZE) throw new IllegalArgumentException("IV must be 16 bytes");
        byte[] counter = Arrays.copyOf(ivCounter, BLOCK_SIZE);
        byte[] out = new byte[input.length];
        int blocks = (input.length + BLOCK_SIZE - 1) / BLOCK_SIZE;
        for (int i = 0; i < blocks; i++) {
            byte[] keystream = GOST_Kuz_Encrypt(counter);
            int off = i * BLOCK_SIZE;
            int remain = Math.min(BLOCK_SIZE, input.length - off);
            for (int j = 0; j < remain; j++) {
                out[off + j] = (byte) (input[off + j] ^ keystream[j]);
            }
            incCounter(counter);
        }
        return out;
    ...
    {
        byte[] key1 = key_1_default;
        byte[] key2 = key_2_default;

        byte[] iv = new byte[16];
        SecureRandom rnd = new SecureRandom();
        rnd.nextBytes(iv);
        byte[] plainBytes = stringToUtf8(plaintext);
        byte[] cipher = GOST_Kuz_CTR_Process(iv, plainBytes);
        return printHexBinary(cipher);
    }
